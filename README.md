...
If a broker thread cannot be created at all, the failure is reported to `error_handler` and the affected work fails with `ResourceLimitError` rather than waiting without a deadline: a spawn or brokered call is refused, a timer is dropped, a pending relaunch counts as a failure, and children of a failed actor are stopped inline. Nothing propagates into other threads or into `stop`. A relaunch refused by the preflight counts against `max_restarts` like a crash; with short-lived runs that is preferable to an unbounded wait for headroom.
